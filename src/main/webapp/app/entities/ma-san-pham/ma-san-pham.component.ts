...
currentAccount: any;
    ...
    loadAll() {
        this.maSanPhamService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<MaSanPham[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/ma-san-pham'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/ma-san-pham', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
